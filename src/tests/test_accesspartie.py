...
def test_access_partie_depuis_sql(capfd):
            ...
            for table in partie.tables:
                for siege in table.sieges:
                    if siege.id_joueur == joueur.id_joueur:
                        jp = JoueurPartie(joueur=joueur, siege=siege, solde_partie=int(joueur.credit))
                        print(f"{jp.joueur.pseudo} rejoint une table avec {jp.solde_partie.get()} jetons.")
                        break
        else:
    ...
    for table in partie.tables:
        ...
        print(f"Table {table.id_table} - Blind {table.blind_initial.get()} - Sièges occupés : {occupes}/{len(table.sieges)}")
