...
def test_creer_table_ok(mock_table_dao, mock_access_partie):
    ...
    nb_sieges = 5
    blind_initial = 10
    mock_table = Table(id_table=1, nom_table="Table1", nb_sieges_max=nb_sieges, blind_initial=blind_initial)
...
def test_rejoindre_table_ok(mock_table_dao):
    ...
    id_table = 1
    mock_table = Table(id_table=id_table, nom_table="Table1", nb_sieges_max=5, blind_initial=10)
...
def test_rejoindre_table_ko_table_remplie(mock_table_dao):
    ...
    id_table = 1
    mock_table = Table(id_table=id_table, nom_table="Table1", nb_sieges_max=5, blind_initial=10)
    mock_table.sieges = [MagicMock(occupe=True, id_joueur=1) for _ in range(5)]
    mock_table_dao["trouver_par_id"].return_value = mock_table
...
def test_lister_tables_disponibles(mock_table_dao):
    # GIVEN
    mock_tables = [
        Table(id_table=1, nom_table="Table1", nb_sieges_max=5, blind_initial=10),
        Table(id_table=2, nom_table="Table2", nb_sieges_max=5, blind_initial=10),
    ]
